...
namespace DPS.SuperAdmin
{
    public partial class ReVerifyPayment : System.Web.UI.Page
    {
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            ...
            if(dt.Rows.Count>0)
            {
                try
                {
                    ...
                    {
                        string currenddb = Session["databaseName"].ToString();
                        Session["databaseName"] = currenddb;
        ...
        public DataTable GetPaymentTransactionBYTransactionID(string transactionId)
        {
            ...
            string query = "SELECT Count([OnlineRefNo])  FROM [FeeTransactionOnline] where OnlineRefNo='"+transactionId+"' ";
            SqlCommand com = new SqlCommand(query, con);
            SqlDataAdapter sda = new SqlDataAdapter(com);
            DataTable dt2 = new DataTable();
            sda.Fill(dt2);
